...
Preserve uses a provider that wraps your application to provide your instance's context. See below examples for the app router and pages router.
...
The Search component can be installed anywhere in your application. It wraps a "trigger", which when clicked will pop up the menu. This means you can style the "trigger" element however you'd like for the menu to pop up.
...
export default function ComponentWithSearch({ children }: { children: React.ReactNode }) {
  ...
  return (
    <Search callbacks={callbacks} mode={'dev'} theme={'dark'}>
